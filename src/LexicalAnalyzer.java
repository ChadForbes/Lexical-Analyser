...
public class LexicalAnalyzer 
{
	 ...
	 *             if line is null or lineNumber <= 0 postcondition: source code
	 *             line has been processed with all tokens placed in the token list
	 */
	 ...
	 *             if lexeme is null
	 * @throws LexicalException
	 *             if lexeme is not a valid representation of a token
	 */
	private TokenType getTokenType(String lexeme, int rowNumber, int columnNumber) throws LexicalException 
	{
		...
		if (Character.isDigit(lexeme.charAt(0)))
			if (allDigits(lexeme))
				...
				throw new LexicalException(
						"literal integer expecated " + " at row " + rowNumber + " and column " + columnNumber);
		else if (Character.isLetter(lexeme.charAt(0))) 
		{
			if (lexeme.length() == 1)
				...
				throw new LexicalException("invalid lexeme " + " at row " + rowNumber + " and column " + columnNumber);
		} 
	 ...
	 * @throws IllegalArgumertException
	 *             if lexeme is null
	 */
